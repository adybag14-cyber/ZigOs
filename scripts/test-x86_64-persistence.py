#!/usr/bin/env python3
"""Prove ZigOs NVMe persistence across two independent x86-64 boots."""
...
def main() -> int:
    ...
    try:
        ...
        if after_second_hash == after_first_hash:
            raise RuntimeError("boot 2 did not commit a new NVMe generation")
        header_a = parse_header(image, data_first_lba, 0)
        header_b = parse_header(image, data_first_lba, 1)
        ...
        summary = {
            ...
            "after_boot2_sha256": after_second_hash,
        ...
        print("Persistent x86-64 NVMe two-boot session passed.")
        print(f"  boot 1: slot A generation {header_a.generation}, records {header_a.record_count}")
        print(f"  boot 2: slot B generation {header_b.generation}, records {header_b.record_count}")
        print(f"  image SHA-256: {initial_hash[:12]} -> {after_first_hash[:12]} -> {after_second_hash[:12]}")
        return 0
    finally:
        try:
            lock.unlink()
