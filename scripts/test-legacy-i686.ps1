...
$readyMarker = 'ZigOs i686 shell ready: prompt zigos> commands help ls mem ticks disk cat HELLO.TXT run INIT.ELF run CAT.ELF ps exit'
$finalMarker = 'ZigOs i686 shell verified: commands 0x00000009 unknown 0x00000000 exit yes'
$commandPlan = @(
    @{ Command = 'help'; Expect = 'commands: help ls mem ticks disk cat HELLO.TXT run INIT.ELF run CAT.ELF ps exit' },
    @{ Command = 'ls'; Expect = 'CAT.ELF 0x000001FE' },
    @{ Command = 'mem'; Expect = 'frames-free 0x00001ECF heap-free 0x00007FF0 heap-base 0x00107000' },
    @{ Command = 'ticks'; Expect = 'ticks 0x00000013 PIT-Hz 0x00000064' },
    @{ Command = 'disk'; Expect = 'model QEMU HARDDISK sectors 0x00001000 FAT12 yes HELLO.TXT-bytes 0x00000056 INIT.ELF-bytes 0x000001A7 CAT.ELF-bytes 0x000001FE' },
    ...
    @{ Command = 'run CAT.ELF'; Expect = 'process PID 0x00000005 CAT.ELF exited 0x00000044 syscalls 0x00000005 read-bytes 0x00000056' },
    @{ Command = 'ps'; Expect = 'PID 0x00000005 EXITED 0x00000044 CAT.ELF' },
...
$vfsReadyMarker = 'ZigOs i686 VFS/process ready: mount FAT12 root-files 0x00000003 fd-capacity 0x00000004 probe-fd 0x00000000 split-read 0x00000020/0x00000036 owner-probe-fd 0x00000000 owner-denied yes process-capacity 0x00000008 PID1 exited 0x00000033'
$userSchedulerMarker = 'ZigOs i686 user scheduler verified: policy preemptive-CPL3 tasks 0x00000002 address-spaces 0x00000002 switches 0x00000007 quanta 0x00000003/0x00000003 tick-delta 0x00000007 CR3-distinct yes kernel-stacks distinct shared-VA 0x00400100 tags 0x11/0x22 active yes frames-restored yes'
$userVfsMarker = 'ZigOs i686 user VFS syscalls verified: open 0x00000001 reads 0x00000001 read-bytes 0x00000056 closes 0x00000001 write-bytes 0x00000056 exit 0x00000044 owner-isolation yes explicit-close yes cleanup-closes 0x00000000 heap-restored yes'
$vfsFinalMarker = 'ZigOs i686 VFS/process verified: opens 0x00000006 reads 0x00000006 closes 0x00000006 processes 0x00000005 last-pid 0x00000005 last-exit 0x00000044 descriptors-closed yes'
...
$debugMarkers = @(
    ...
    'INIT.ELF executed in ring3 via FAT12.', $elfMarker, $vfsReadyMarker, $userSchedulerMarker,
    $readyMarker,
    'commands: help ls mem ticks disk cat HELLO.TXT run INIT.ELF run CAT.ELF ps exit',
    'HELLO.TXT 0x00000056', 'INIT.ELF 0x000001A7', 'CAT.ELF 0x000001FE',
    'frames-free 0x00001ECF heap-free 0x00007FF0 heap-base 0x00107000',
    'ticks 0x00000013 PIT-Hz 0x00000064',
    'model QEMU HARDDISK sectors 0x00001000 FAT12 yes HELLO.TXT-bytes 0x00000056 INIT.ELF-bytes 0x000001A7 CAT.ELF-bytes 0x000001FE',
    'ZigOs legacy FAT12 filesystem is online.',
    'Loaded through ATA PIO by the i686 kernel.',
    'process PID 0x00000004 INIT.ELF exited 0x00000033 syscalls 0x00000003',
    $userVfsMarker, 'process PID 0x00000005 CAT.ELF exited 0x00000044 syscalls 0x00000005 read-bytes 0x00000056',
    'PID 0x00000001 EXITED 0x00000033 INIT.ELF',
    'PID 0x00000002 EXITED 0x00000040 WORKA.BIN', 'PID 0x00000003 EXITED 0x00000041 WORKB.BIN',
    'PID 0x00000004 EXITED 0x00000033 INIT.ELF', 'PID 0x00000005 EXITED 0x00000044 CAT.ELF',
...
$serialMarkers = @(
    ...
    'ZigOs ring3 syscall write verified.', $syscallMarker, 'INIT.ELF executed in ring3 via FAT12.', $elfMarker, $vfsReadyMarker, $userSchedulerMarker, $readyMarker,
    'help', 'commands: help ls mem ticks disk cat HELLO.TXT run INIT.ELF run CAT.ELF ps exit',
    'ls', 'HELLO.TXT 0x00000056', 'INIT.ELF 0x000001A7', 'CAT.ELF 0x000001FE',
    'mem', 'frames-free 0x00001ECF heap-free 0x00007FF0 heap-base 0x00107000',
    'ticks', 'ticks 0x00000013 PIT-Hz 0x00000064',
    'disk', 'model QEMU HARDDISK sectors 0x00001000 FAT12 yes HELLO.TXT-bytes 0x00000056 INIT.ELF-bytes 0x000001A7 CAT.ELF-bytes 0x000001FE',
    'cat HELLO.TXT', 'ZigOs legacy FAT12 filesystem is online.',
    'Loaded through ATA PIO by the i686 kernel.',
    'run INIT.ELF', 'process PID 0x00000004 INIT.ELF exited 0x00000033 syscalls 0x00000003',
    'run CAT.ELF', $userVfsMarker, 'process PID 0x00000005 CAT.ELF exited 0x00000044 syscalls 0x00000005 read-bytes 0x00000056',
    'ps', 'PID 0x00000001 EXITED 0x00000033 INIT.ELF',
    'PID 0x00000002 EXITED 0x00000040 WORKA.BIN', 'PID 0x00000003 EXITED 0x00000041 WORKB.BIN',
    'PID 0x00000004 EXITED 0x00000033 INIT.ELF', 'PID 0x00000005 EXITED 0x00000044 CAT.ELF',
