...
try {
    ...
    $required = @(
        ...
        'writeback: active no requests/completions/passes 1/1/1 immediate/durable/clean/unsupported/failures/stale 1/0/0/0/0/0 pages queued/completed 3/3',
        'cache pressure: free before/after ',
        $(if ($Network) { 'entries before/after 16/12 reclaimed 4 dirty retained 12' } else { 'entries before/after 15/11 reclaimed 4 dirty retained 11' }),
        ...
        $(if ($Network) { ' backing alloc/release/fail 55/55/0/0' } else { ' backing alloc/release/fail 50/50/0/0' }),
        ...
        $(if ($Network) { 'alloc/free 334/334 failed/rejected 0/0 clean yes' } else { 'alloc/free 298/298 failed/rejected 0/0 clean yes' }),
        ...
        $(if ($Network) { 'reclaimed 279 stale-contexts-swept 0 allocator alloc/release/retains 279/279/0' } else { 'reclaimed 248 stale-contexts-swept 0 allocator alloc/release/retains 248/248/0' }),
