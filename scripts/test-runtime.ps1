...
try {
    ...
    $required = @(
        ...
        'zig-sdk: startup/argv/abi/files/vm/file-mmap/errno/fsync/fdatasync/readv/writev/mount/umount/tmpfs/statfs/stattimes/statowner/umask/setid-metadata/flock/lockrange passed',
        'exec: PID 16 state zombie status 0x56',
        'c-sdk: start',
        'c-sdk: argc/argv passed',
        'c-sdk: ABI 1.19 discovery passed',
        ...
        'c-sdk: generated header/library/device/ioctl/stat/statfs/stattimes/statowner/umask/setid-metadata/flock/lockrange/directory-openat/fsync/fdatasync/symlink/readlink/link/nlink/fallocate/sparse/readv/writev passed',
        ...
        $(if ($Network) { 'entries before/after 16/9 reclaimed 7 dirty retained 9' } else { 'entries before/after 15/8 reclaimed 7 dirty retained 8' }),
        ...
        $(if ($Network) { ' backing alloc/release/fail 59/59/0/0' } else { ' backing alloc/release/fail 54/54/0/0' }),
        $(if ($Network) { '/1/7 shutdown-release 9' } else { '/1/7 shutdown-release 8' }),
        ...
        $(if ($Network) { 'alloc/free 342/342 failed/rejected 0/0 clean yes' } else { 'alloc/free 306/306 failed/rejected 0/0 clean yes' }),
        ...
        $(if ($Network) { 'reclaimed 283 stale-contexts-swept 0 allocator alloc/release/retains 283/283/0' } else { 'reclaimed 252 stale-contexts-swept 0 allocator alloc/release/retains 252/252/0' }),
