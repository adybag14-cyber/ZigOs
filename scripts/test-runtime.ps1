...
try {
    ...
    $required = @(
        ...
        'zig-sdk: startup/argv/abi/files/vm/file-mmap/errno/fsync/fdatasync/readv/writev/mount/umount/tmpfs/statfs passed',
        'exec: PID 16 state zombie status 0x56',
        'c-sdk: start',
        'c-sdk: argc/argv passed',
        'c-sdk: ABI 1.14 discovery passed',
        'c-sdk: generated header/library/device/ioctl/stat/statfs/directory-openat/fsync/fdatasync/symlink/readlink/link/nlink/fallocate/sparse/readv/writev passed',
        'exec: PID 17 state zombie status 0x57',
        'serial COM1 online',
        'fsck ramfs/persist: clean',
        'writeback scheduled: node ',
        'writeback: active no requests/completions/passes 1/1/1 immediate/durable/clean/unsupported/failures/stale 1/0/0/0/0/0 pages queued/completed 4/4',
        'cache pressure: free before/after ',
        $(if ($Network) { 'entries before/after 16/11 reclaimed 5 dirty retained 11' } else { 'entries before/after 15/10 reclaimed 5 dirty retained 10' }),
        ...
        $(if ($Network) { 'fdtest counters: dup 2 inherited 62 cloexec 1 blocked 3/1 wakeups 3/1' } else { 'fdtest counters: dup 2 inherited 56 cloexec 1 blocked 3/1 wakeups 3/1' }),
        ...
        $(if ($Network) { ' backing alloc/release/fail 56/56/0/0' } else { ' backing alloc/release/fail 51/51/0/0' }),
        $(if ($Network) { '/1/5 shutdown-release 11' } else { '/1/5 shutdown-release 10' }),
        ...
        $(if ($Network) { 'alloc/free 336/336 failed/rejected 0/0 clean yes' } else { 'alloc/free 300/300 failed/rejected 0/0 clean yes' }),
        ...
        $(if ($Network) { 'reclaimed 280 stale-contexts-swept 0 allocator alloc/release/retains 280/280/0' } else { 'reclaimed 249 stale-contexts-swept 0 allocator alloc/release/retains 249/249/0' }),
