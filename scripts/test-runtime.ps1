...
try {
    ...
    $required = @(
        ...
        'zig-sdk: startup/argv/abi/files/vm/file-mmap/errno/fsync/fdatasync/readv/writev/mount/umount/tmpfs/statfs/stattimes/statowner/umask/setid-metadata/flock passed',
        'exec: PID 16 state zombie status 0x56',
        'c-sdk: start',
        'c-sdk: argc/argv passed',
        'c-sdk: ABI 1.18 discovery passed',
        'c-sdk: process umask creation passed',
        'c-sdk: setuid/setgid metadata passed',
        'c-sdk: advisory whole-file flock passed',
        'c-sdk: generated header/library/device/ioctl/stat/statfs/stattimes/statowner/umask/setid-metadata/flock/directory-openat/fsync/fdatasync/symlink/readlink/link/nlink/fallocate/sparse/readv/writev passed',
        'exec: PID 17 state zombie status 0x57',
        'serial COM1 online',
        'fsck ramfs/persist: clean',
        'writeback scheduled: node ',
        'writeback: active no requests/completions/passes 1/1/1 immediate/durable/clean/unsupported/failures/stale 1/0/0/0/0/0 pages queued/completed 5/5',
        'cache pressure: free before/after ',
        $(if ($Network) { 'entries before/after 16/10 reclaimed 6 dirty retained 10' } else { 'entries before/after 15/9 reclaimed 6 dirty retained 9' }),
        ...
        $(if ($Network) { 'fdtest counters: dup 4 inherited 62 cloexec 1 blocked 3/1 wakeups 3/1' } else { 'fdtest counters: dup 4 inherited 56 cloexec 1 blocked 3/1 wakeups 3/1' }),
        ...
        $(if ($Network) { ' backing alloc/release/fail 58/58/0/0' } else { ' backing alloc/release/fail 53/53/0/0' }),
        $(if ($Network) { '/1/6 shutdown-release 10' } else { '/1/6 shutdown-release 9' }),
        ...
        'faults 1',
        $(if ($Network) { 'ZigOs persistent descriptors: namespaces 1 fds 3 open 3 terminals 3 vfs 0 pipes 0 dup/inherited/cloexec 4/62/1 blocked 3/1 wakeups 3/1' } else { 'ZigOs persistent descriptors: namespaces 1 fds 3 open 3 terminals 3 vfs 0 pipes 0 dup/inherited/cloexec 4/56/1 blocked 3/1 wakeups 3/1' }),
        ...
        $(if ($Network) { 'alloc/free 340/340 failed/rejected 0/0 clean yes' } else { 'alloc/free 304/304 failed/rejected 0/0 clean yes' }),
        ...
        $(if ($Network) { 'reclaimed 282 stale-contexts-swept 0 allocator alloc/release/retains 282/282/0' } else { 'reclaimed 251 stale-contexts-swept 0 allocator alloc/release/retains 251/251/0' }),
