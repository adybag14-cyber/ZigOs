...
try {
    ...
    $required = @(
        ...
        $(if ($Network) { 'fdtest counters: dup 2 inherited 56 cloexec 1 blocked 3/1 wakeups 3/1' } else { 'fdtest counters: dup 2 inherited 53 cloexec 1 blocked 3/1 wakeups 3/1' }),
        'FD KIND       MODE OFD      REFS FLAGS OFFSET/BUFFERED',
        '0 terminal',
        '1 terminal',
        '2 terminal',
        $(if ($Network) { 'ZigOs persistent runtime shutdown: commands 45 failed 0' } else { 'ZigOs persistent runtime shutdown: commands 44 failed 0' }),
        'ZigOs persistent VFS:',
        'ZigOs persistent processes:',
        'faults 1',
        $(if ($Network) { 'ZigOs persistent descriptors: namespaces 1 fds 3 open 3 terminals 3 vfs 0 pipes 0 dup/inherited/cloexec 2/56/1 blocked 3/1 wakeups 3/1' } else { 'ZigOs persistent descriptors: namespaces 1 fds 3 open 3 terminals 3 vfs 0 pipes 0 dup/inherited/cloexec 2/53/1 blocked 3/1 wakeups 3/1' }),
        ...
        $(if ($Network) { 'peak 32 alloc/free 247/247 failed/rejected 0/0 clean yes' } else { 'peak 32 alloc/free 231/231 failed/rejected 0/0 clean yes' }),
        'ZigOs permanent userspace: page-limit 4096 used 0',
        $(if ($Network) { 'launches/exits/faults 15/13/1' } else { 'launches/exits/faults 14/12/1' }),
        $(if ($Network) { 'reclaimed 247 allocator alloc/release/retains 247/247/0' } else { 'reclaimed 231 allocator alloc/release/retains 231/231/0' }),
