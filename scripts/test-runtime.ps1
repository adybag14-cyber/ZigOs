...
try {
    ...
    if ($Network) {
        $arguments += @('-netdev', "user,id=net0,tftp=$tftpPath,hostfwd=tcp:127.0.0.1:39002-10.0.2.15:39002", '-device', 'e1000e,netdev=net0,mac=52:54:00:12:34:56')
    } else {
        $arguments += @('-net', 'none')
    }
    ...
    $commands = @(
        ...
        'exec /bin/runtime-sleep.elf',
        'crash',
        'pipex',
        'spawn /bin/runtime-sleep.elf',
        'wait 8',
        'spawn /bin/spin.elf',
        'ps',
        'jobs',
    ...
    foreach ($command in $commands) {
        ...
        if ($Network -and $command -eq 'exec /bin/socket.elf') {
            ...
            if (-not $tcpFixtureAccept -or -not $tcpFixtureAccept.AsyncWaitHandle.WaitOne([TimeSpan]::FromSeconds(5))) {
                throw 'The G305 active TCP fixture did not reach the loopback host listener.'
            }
            $tcpFixtureClient = $tcpFixtureListener.EndAcceptTcpClient($tcpFixtureAccept)
            $tcpFixtureAccept = $null
            $tcpShutdownStream = $tcpFixtureClient.GetStream()
            $tcpShutdownStream.ReadTimeout = 5000
            $tcpShutdownByte = [byte[]]::new(1)
            try {
                $tcpShutdownRead = $tcpShutdownStream.Read($tcpShutdownByte, 0, 1)
            } catch {
                throw "The G305 host did not observe TCP write-half EOF: $($_.Exception.Message)"
            }
            if ($tcpShutdownRead -ne 0) { throw "The G305 host read $tcpShutdownRead bytes instead of EOF after guest SHUT_WR." }
            Write-Host 'G305 TCP shutdown host proof passed: guest SHUT_WR delivered FIN/EOF to the loopback host peer.'
        }
    ...
    Read-SerialAvailable
    if ($Network -and -not $tcpFixtureClient) {
        throw 'The G305 TCP fixture never completed its host-side half-close proof.'
    }
    ...
    $required = @(
        ...
        '[8] CPL3 started /bin/runtime-sleep.elf',
        ...
        ' page-write tickets/outstanding 378/0',
