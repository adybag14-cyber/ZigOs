...
if ($Network) {
    ...
    }    if (-not [regex]::IsMatch($output, 'NTP service verified: source (HPET|ACPI PM timer), frequency/bits [1-9][0-9]*/(24|32|64), socket 2/45/49186, bootstrap rejected/state preserved yes/yes, initial timestamp 0xEEF4507F40000000, intervals 1/2, initial TX 34/5/6, early no-TX yes, retry TX 35/6/7 transmissions 2 timestamp preserved yes, first sample [1-9][0-9]* time 1800000000/0x80000000 deadline [1-9][0-9]*, pre-anchor idle preserved yes, before refresh no-TX yes, refresh timestamp 0x[0-9A-F]{16} automatic yes, refresh TX 36/7/0, second sample [1-9][0-9]* time 1800000002/0x80000000, counts 2/1/2, close/inactive preserved yes/yes, final IP/TX 37/0, submissions 3, completions TX/RX 64/64/22, endpoints/cursor 2/49187, ingress 82/82, dispatch 71/70')) {
        throw 'The owned NTP service did not reject an absent bootstrap transactionally, preserve retry originate time, derive refresh time from its projected clock, enforce deadlines, and shut down cleanly.'    }    if (-not $output.Contains('NTP timestamp verified: base/anchor 0xEEF4508080000000/0xEEF4508080000000, quarter/rollover 0xEEF45080C0000000/0xEEF4508140000000, maximum 0xFFFFFFFFFFFFFFFF, rejects unsynchronized/backward/overflow yes/yes/yes')) {
