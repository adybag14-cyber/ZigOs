...
def generate_c(spec: dict) -> str:
    ...
    for name, value in spec.get("spawn_io", {}).items():
        lines.append(f"#define ZIGOS_SPAWN_IO_{name.upper()} UINT16_C({value})\n")
    for name, value in spec.get("directory_fds", {}).items():
...
def generate_c_abi_description(spec: dict) -> str:
    """Generate the header-only C ABI contract without syscall-wrapper declarations."""
    full = generate_c(spec)
    start_marker = "#ifndef ZIGOS_ABI_H\n#define ZIGOS_ABI_H\n"
    end_marker = "#endif /* ZIGOS_ABI_H */\n"
    start = full.find(start_marker)
    end = full.find(end_marker, start)
    if start < 0 or end < 0:
        raise ValueError("generated C ABI description markers missing")
    abi_block = full[start:end]
    return (
        "/* Generated by scripts/generate-abi.py from abi/zigos-abi.json. Do not edit. */\n"
        + abi_block
        + '#ifdef __cplusplus\nextern "C" {\n#endif\n'
...
def main() -> int:
    ...
    c_abi_description = generate_c_abi_description(spec)
    ...
    write_or_check(root / "sdk" / "c" / "include" / "zigos_abi.h", c_abi_description, args.check)
