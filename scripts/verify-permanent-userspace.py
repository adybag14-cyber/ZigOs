...
def main() -> int:
    ...
    require(normal_boot_test, "alloc/free 138/138 cache-released 13 storage persistent clean yes", "normal QEMU gate requires exact physical reclamation and persistent mode")
    ...
    require(io_error_boot_test, "10908/5203/0/0/0 lock tickets/outstanding 4/0 quarantine state/reason/events no/none/0 clean yes", "G247 EIO recovery retains measured private-build FAT ownership")
    ...
    require(readonly_remount_boot_test, "10908/5203/0/0/0 lock tickets/outstanding 1/0 quarantine state/reason/events no/none/0 clean yes", "G247 contained persistent-write profile retains measured private-build FAT ownership")
    ...
    require(runtime_test, "11007/5104/0/0/0 lock tickets/outstanding 5/0 quarantine state/reason/events no/none/0 clean yes", "both G247 diagnostic QEMU profiles require exact global FAT ownership with zero classified corruption")
    ...
    require(normal_boot_test, "10908/5203/0/0/0 lock tickets/outstanding 1/0 quarantine state/reason/events no/none/0 clean yes", "G250 normal QEMU requires exact FAT ownership with zero classified corruption")
    ...
    require(readme, "Size:    5,633,536 bytes", "README carries the reproducible G258 ABI 1.20 EFI size")
    require(readme, "SHA-256: 542816F3A62E84F30641AA079A69B39A595A2A525F17702A783169EA2758B6F9", "README carries the reproducible G258 ABI 1.20 EFI hash")
    ...
    require(runtime_test, "bytes 5633604 metadata/file/block reads 112/4/114 failures 0 clusters claimed/free/loop/cross/range 11007/5104/0/0/0", "G247 diagnostic harness freezes the measured FAT geometry")
    require(normal_boot_test, "10908/5203/0/0/0 lock tickets/outstanding 1/0", "G250 normal harness freezes the measured FAT geometry")
    require(io_error_boot_test, "bytes 5582916 metadata/file/block reads 112/3/113 failures 1 clusters claimed/free/loop/cross/range 10908/5203/0/0/0", "G247 read-EIO harness freezes its private fault-injection geometry")
    require(readonly_remount_boot_test, "bytes 5582916 metadata/file/block reads 111/0/111 failures 0 clusters claimed/free/loop/cross/range 10908/5203/0/0/0", "G247 write-EIO harness freezes its private fault-injection geometry")
    ...
    require(sdk_shell, "last_status = execute(&command, last_status);", "G258 updates status only after parsed command execution")
    ...
    require(roadmap, "212 complete, 288 open", "roadmap arithmetic includes G258")
    ...
    if len(roadmap_items) != 500 or sum(item == "x" for item in roadmap_items) != 212:
        raise SystemExit("roadmap must contain 500 goals with exactly 212 complete after G258")
