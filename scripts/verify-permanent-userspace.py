...
def main() -> int:
    ...
    require(normal_boot_test, "alloc/free 466/466 cache-released 13 storage persistent clean yes", "normal QEMU gate requires exact G272 physical reclamation and persistent mode")
    ...
    require(io_error_boot_test, "10954/5157/0/0/0 lock tickets/outstanding 4/0 quarantine state/reason/events no/none/0 clean yes", "G272 EIO recovery retains measured private-build FAT ownership")
    ...
    require(readonly_remount_boot_test, "10954/5157/0/0/0 lock tickets/outstanding 1/0 quarantine state/reason/events no/none/0 clean yes", "G272 contained persistent-write profile retains measured private-build FAT ownership")
    ...
    require(runtime_test, "11054/5057/0/0/0 lock tickets/outstanding 5/0 quarantine state/reason/events no/none/0 clean yes", "both G247 diagnostic QEMU profiles require exact global FAT ownership with zero classified corruption")
    ...
    require(normal_boot_test, "10954/5157/0/0/0 lock tickets/outstanding 1/0 quarantine state/reason/events no/none/0 clean yes", "G266 normal QEMU requires exact FAT ownership with zero classified corruption")
    ...
    require(runtime_test, "page-write tickets/outstanding 194/0", "both diagnostic QEMU profiles require the measured page-writer ticket total with no outstanding lock")
    ...
    require(readme, "Size:    5,657,600 bytes", "README carries the reproducible G273 ABI 1.24 EFI size")
    require(readme, "SHA-256: 10018FE4B2BFCFCC8E744A2768F2505CBDD3B5DAC53270D0D091CA319FC13988", "README carries the reproducible G273 ABI 1.24 EFI hash")
    ...
    require(runtime_test, "bytes 5657668 metadata/file/block reads 112/4/114 failures 0 clusters claimed/free/loop/cross/range 11054/5057/0/0/0", "G247 diagnostic harness freezes the measured FAT geometry")
    require(normal_boot_test, "10954/5157/0/0/0 lock tickets/outstanding 1/0", "G272 normal harness freezes the measured FAT geometry")
    require(io_error_boot_test, "bytes 5606468 metadata/file/block reads 112/3/113 failures 1 clusters claimed/free/loop/cross/range 10954/5157/0/0/0", "G264 read-EIO harness freezes its private fault-injection geometry")
    require(readonly_remount_boot_test, "bytes 5606468 metadata/file/block reads 111/0/111 failures 0 clusters claimed/free/loop/cross/range 10954/5157/0/0/0", "G264 write-EIO harness freezes its private fault-injection geometry")
    ...
    require(roadmap, "227 complete, 273 open", "roadmap arithmetic includes G273")
    ...
    if len(roadmap_items) != 500 or sum(item == "x" for item in roadmap_items) != 227:
        raise SystemExit("roadmap must contain 500 goals with exactly 227 complete after G273")
