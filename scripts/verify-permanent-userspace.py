...
def main() -> int:
    ...
    require(normal_boot_test, "alloc/free 268/268 cache-released 16 storage persistent clean yes", "normal QEMU gate requires exact G264 physical reclamation and persistent mode")
    ...
    require(readme, "Size:    5,649,920 bytes", "README carries the reproducible G264 ABI 1.23 EFI size")
    require(readme, "SHA-256: AA57E81C677FE304BD6A98C86AAA5E17AD0E9790ABF28D8A12E71C646ED61D50", "README carries the reproducible G264 ABI 1.23 EFI hash")
    ...
    require(sdk_shell, "const shell_group = zigos.ttyForegroundProcessGroup(0)", "G263 shell records its kernel-reported foreground group before handoff")
    require(sdk_shell, "zigos.ttySetForegroundProcessGroup(0, process_group)", "G263 shell hands the TTY to a complete pipeline group")
    require(sdk_shell, "zigos.waitProcessGroup(process_group, false, &status)", "G263 shell reaps successful pipelines by PGID")
    ...
    require(normal_boot_test, 'wait_for(client, process, serial, b"pipeline foreground group 11"', "normal QEMU observes kernel-backed foreground PGID 11")
    ...
    require(normal_boot_test, '"pipeline foreground restored 2"', "normal QEMU requires foreground restoration to shell PGID 2")
    ...
    require(roadmap, "218 complete, 282 open", "roadmap arithmetic includes G264")
    ...
    if len(roadmap_items) != 500 or sum(item == "x" for item in roadmap_items) != 218:
        raise SystemExit("roadmap must contain 500 goals with exactly 218 complete after G264")
