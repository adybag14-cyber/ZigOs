...
pub fn build(b: *std.Build) void {
    ...
    const kill_module = b.createModule(.{
        .root_source_file = b.path("sdk/zig/kill.zig"),
        .target = sdk_target,
        .optimize = .ReleaseSmall,
        .strip = true,
        .code_model = .large,
    ...
    kill_module.addObjectFile(b.path("build/sdk/syscall.o"));
    const userspace_kill = b.addExecutable(.{
        .name = "kill",
        .root_module = kill_module,
    });
    userspace_kill.entry = .{ .symbol_name = "_start" };
    userspace_kill.setLinkerScript(b.path("sdk/zig/linker.ld"));
    userspace_kill.step.dependOn(&assets.step);

    const sleep_module = b.createModule(.{
        .root_source_file = b.path("sdk/zig/sleep.zig"),
        .target = sdk_target,
        .optimize = .ReleaseSmall,
        .strip = true,
        .code_model = .large,
    ...
    sleep_module.addObjectFile(b.path("build/sdk/syscall.o"));
    const userspace_sleep = b.addExecutable(.{
        .name = "sleep",
        .root_module = sleep_module,
    });
    userspace_sleep.entry = .{ .symbol_name = "_start" };
    userspace_sleep.setLinkerScript(b.path("sdk/zig/linker.ld"));
    userspace_sleep.step.dependOn(&assets.step);

    const mount_module = b.createModule(.{
        .root_source_file = b.path("sdk/zig/mount.zig"),
        .target = sdk_target,
        .optimize = .ReleaseSmall,
        .strip = true,
        .code_model = .large,
    ...
    mount_module.addObjectFile(b.path("build/sdk/syscall.o"));
    const userspace_mount = b.addExecutable(.{
        .name = "mount",
        .root_module = mount_module,
    });
    userspace_mount.entry = .{ .symbol_name = "_start" };
    userspace_mount.setLinkerScript(b.path("sdk/zig/linker.ld"));
    userspace_mount.step.dependOn(&assets.step);

    const df_module = b.createModule(.{
        .root_source_file = b.path("sdk/zig/df.zig"),
        .target = sdk_target,
        .optimize = .ReleaseSmall,
        .strip = true,
        .code_model = .large,
    ...
    df_module.addObjectFile(b.path("build/sdk/syscall.o"));
    const userspace_df = b.addExecutable(.{
        .name = "df",
        .root_module = df_module,
    });
    userspace_df.entry = .{ .symbol_name = "_start" };
    ...
    const uname_module = b.createModule(.{
        .root_source_file = b.path("sdk/zig/uname.zig"),
        .target = sdk_target,
        .optimize = .ReleaseSmall,
        .strip = true,
        .code_model = .large,
    ...
    uname_module.addObjectFile(b.path("build/sdk/syscall.o"));
    const userspace_uname = b.addExecutable(.{
        .name = "uname",
        .root_module = uname_module,
    });
    userspace_uname.entry = .{ .symbol_name = "_start" };
    userspace_uname.setLinkerScript(b.path("sdk/zig/linker.ld"));
    userspace_uname.step.dependOn(&assets.step);

    const env_module = b.createModule(.{
        .root_source_file = b.path("sdk/zig/env.zig"),
        .target = sdk_target,
        .optimize = .ReleaseSmall,
        .strip = true,
        .code_model = .large,
    ...
    env_module.addObjectFile(b.path("build/sdk/syscall.o"));
    const userspace_env = b.addExecutable(.{
        .name = "env",
        .root_module = env_module,
    });
    userspace_env.entry = .{ .symbol_name = "_start" };
    userspace_env.setLinkerScript(b.path("sdk/zig/linker.ld"));
    userspace_env.step.dependOn(&assets.step);

    const edit_module = b.createModule(.{
        .root_source_file = b.path("sdk/zig/edit.zig"),
        .target = sdk_target,
        .optimize = .ReleaseSmall,
        .strip = true,
        .code_model = .large,
    ...
    edit_module.addObjectFile(b.path("build/sdk/syscall.o"));
    const userspace_edit = b.addExecutable(.{
        .name = "edit",
        .root_module = edit_module,
    });
    userspace_edit.entry = .{ .symbol_name = "_start" };
    userspace_edit.setLinkerScript(b.path("sdk/zig/linker.ld"));
    userspace_edit.step.dependOn(&assets.step);
    ...
    _ = sdk_embed.addCopyFile(userspace_kill.getEmittedBin(), "kill.elf");
    ...
    _ = sdk_embed.addCopyFile(userspace_uname.getEmittedBin(), "uname.elf");
    _ = sdk_embed.addCopyFile(userspace_env.getEmittedBin(), "env.elf");
    _ = sdk_embed.addCopyFile(userspace_edit.getEmittedBin(), "edit.elf");
    ...
    const sdk_embed_module = sdk_embed.add(
        ...
        "pub const sdk = @embedFile(\"sdk.elf\");\n" ++
            ...
            "pub const kill = @embedFile(\"kill.elf\");\n" ++
            ...
            "pub const uname = @embedFile(\"uname.elf\");\n" ++
            "pub const env = @embedFile(\"env.elf\");\n" ++
            "pub const edit = @embedFile(\"edit.elf\");\n" ++
    ...
    const install_kill = b.addInstallFile(
        userspace_kill.getEmittedBin(),
        "artifacts/kill.elf",
    );
    const install_sleep = b.addInstallFile(
        userspace_sleep.getEmittedBin(),
        "artifacts/sleep.elf",
    );
    const install_mount = b.addInstallFile(
        userspace_mount.getEmittedBin(),
        "artifacts/mount.elf",
    );
    const install_df = b.addInstallFile(
    ...
    const install_uname = b.addInstallFile(
        userspace_uname.getEmittedBin(),
        "artifacts/uname.elf",
    );
    const install_env = b.addInstallFile(
        userspace_env.getEmittedBin(),
        "artifacts/env.elf",
    );
    const install_edit = b.addInstallFile(
        userspace_edit.getEmittedBin(),
        "artifacts/edit.elf",
    );
    ...
    b.getInstallStep().dependOn(&install_kill.step);
    ...
    b.getInstallStep().dependOn(&install_uname.step);
    b.getInstallStep().dependOn(&install_env.step);
    ...
    const verify_kill = b.addSystemCommand(&.{ python, "scripts/verify-zigos-sdk-elf.py" });
    verify_kill.addFileArg(userspace_kill.getEmittedBin());
    const verify_sleep = b.addSystemCommand(&.{ python, "scripts/verify-zigos-sdk-elf.py" });
    verify_sleep.addFileArg(userspace_sleep.getEmittedBin());
    const verify_mount = b.addSystemCommand(&.{ python, "scripts/verify-zigos-sdk-elf.py" });
    ...
    const verify_uname = b.addSystemCommand(&.{ python, "scripts/verify-zigos-sdk-elf.py" });
    verify_uname.addFileArg(userspace_uname.getEmittedBin());
    const verify_env = b.addSystemCommand(&.{ python, "scripts/verify-zigos-sdk-elf.py" });
    verify_env.addFileArg(userspace_env.getEmittedBin());
    const verify_edit = b.addSystemCommand(&.{ python, "scripts/verify-zigos-sdk-elf.py" });
    verify_edit.addFileArg(userspace_edit.getEmittedBin());
    ...
    check_step.dependOn(&verify_kill.step);
    ...
    check_step.dependOn(&verify_uname.step);
    check_step.dependOn(&verify_env.step);
    check_step.dependOn(&verify_edit.step);
