...
export fn zigos_user_syscall_handler(
    ...
    if (runtime_user.isActive()) {
        const syscall_number = frame.rax;
        const disposition = runtime_user.handleSyscall(frame, fx_state);
        return runtime_user.validateSyscallReturn(frame, fx_state, disposition, syscall_number);
    }
    syscall_count +%= 1;
