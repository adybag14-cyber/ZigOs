...
pub const NtpMixedResponseBatchReport = struct {
    ...
    wrong_routed: bool,
    valid_routed: bool,
    batch_state: NtpRequestState,
    batch_examined: u16,
    batch_rejected: u16,
    batch_resolved: bool,
    sampled_once: bool,
    no_retry: bool,
    peer_drop_preserved: bool,
...
pub const NtpWrongOriginateReplyReport = struct {
    source_kind: time_reference.Kind,
    frequency_hz: u64,
    counter_bits: u8,
    socket_slot: u16,
    ...
    recovery_switched: bool,
    same_socket_preserved: bool,
    wrong_originate_routed: bool,
    wrong_originate_examined: u16,
    wrong_originate_rejected: u16,
    wrong_originate_pending: bool,
    wrong_originate_no_sample: bool,
    wrong_originate_no_quality: bool,
    wrong_originate_no_step: bool,
    wrong_originate_no_apply: bool,
    wrong_originate_clock_preserved: bool,
    wrong_originate_request_preserved: bool,
    peer_drop_preserved: bool,
    accepted_quality_result: ntp.QualityResult,
    accepted_step_result: ntp.ClockStepResult,
    accepted_sample_tick: u64,
...
pub const NetworkResult = struct {
    ...
    ntp_wrong_originate_reply: NtpWrongOriginateReplyReport,
    ntp_mixed_response_batch: NtpMixedResponseBatchReport,
...
pub fn initializeAndTestNetwork(
    ...
    return .{
        ...
        .ntp_wrong_originate_reply = ntp_wrong_originate_reply,
        .ntp_mixed_response_batch = ntp_mixed_response_batch,
...
fn verifyNtpMixedResponseBatch(
    device: *Device,
    counter: *time_reference.ContinuousCounter,
) ?NtpMixedResponseBatchReport {
    if (device.udp_endpoint_count != 2 or device.next_ephemeral_udp_port != 49_211 or
        device.next_udp_generation != 70 or device.tx_producer != 7 or
        device.next_udp_identification != 124 or device.next_dns_transaction_id != 8 or
        completionQueueEnqueued(&tx_completion_queue) != 151 or
        completionQueueDequeued(&tx_completion_queue) != 151 or
        completionQueueEnqueued(&rx_completion_queue) != 22 or
        device.software_rx_queue.enqueued != 134 or device.software_rx_queue.dequeued != 134 or
        device.packets_dispatched != 122 or device.udp_packets_dispatched != 121 or
        device.peer_mismatch_udp_packets_dropped != 4 or
        counter.frequency_hz == 0 or counter.counter_bits == 0)
    {
        return null;
    }
    ...
    const socket = service.client.socket;
    if (socket.endpoint_index != 2 or socket.generation != 70 or socket.local_port != 49_211 or
        device.udp_endpoint_count != 3 or device.next_ephemeral_udp_port != 49_212 or
        device.next_udp_generation != 71)
        return null;
    ...
    if (initial.state != .awaiting or initial.start_reason != .initial or
        initial_tx.identification != 124 or initial_tx.completion.descriptor_index != 7 or
        initial_tx.completion.next_cursor != 0)
        return null;
    ...
    if (!recovery_started or !recovery_switched or !same_socket_preserved or
        recovery_tx.identification != 125 or recovery_tx.completion.descriptor_index != 0 or
        recovery_tx.completion.next_cursor != 1)
        return null;

    const peer_drops_before = device.peer_mismatch_udp_packets_dropped;
    const wrong_routed = enqueueNtpServiceResponse(
        device,
        socket,
        source_pool.servers[2],
        service.request.client_timestamp +| 1,
        ntp.fixture_server_timestamp + (@as(u64, 2) << 32),
        0x8F01,
        0xFF01,
    );
    const valid_routed = enqueueNtpServiceResponse(
        device,
        socket,
        source_pool.servers[2],
        service.request.client_timestamp,
    ...
    if (!wrong_routed or !valid_routed or device.packets_dispatched != 125 or
        device.udp_packets_dispatched != 124 or device.peer_mismatch_udp_packets_dropped != peer_drops_before)
        return null;

    const retries_before_batch = service.retries;
    const accepted = stepNtpServiceAutomatic(device, &service, counter, recovery_tick, 0, 2) orelse return null;
    ...
    const batch_resolved = accepted.poll.state == .resolved and accepted.poll.response != null;
    const sampled_once = accepted.sample_tick != null and accepted.quality_result == .accepted and
        accepted.step_result == .accepted and accepted.apply_result == .accepted;
    const no_retry = accepted.transmit == null and !accepted.retried and service.retries == retries_before_batch and
        service.request.transmissions == 1;
    const peer_drop_preserved = device.peer_mismatch_udp_packets_dropped == peer_drops_before;
    const accepted_state_clean = accepted.state == .idle and accepted_quality_result == .accepted and
        ...
        accepted.poll.examined == 2 and accepted.poll.rejected == 1 and batch_resolved and sampled_once and no_retry and
        ...
        service.source_rotations == 3 and service.recovery_successes == 1 and peer_drop_preserved;
    ...
    const rxe = completionQueueEnqueued(&rx_completion_queue);
    const overflow = completionQueueOverflow(&tx_completion_queue) + completionQueueOverflow(&rx_completion_queue);
    if (device.udp_endpoint_count != 2 or device.next_ephemeral_udp_port != 49_212 or
        device.next_udp_generation != 71 or device.next_udp_identification != 126 or
        device.next_dns_transaction_id != 8 or device.tx_producer != 1 or
        device.tx_submissions != submissions_before + 2 or txe != 153 or txd != 153 or rxe != 22 or
        overflow != 0 or device.software_rx_queue.enqueued != 137 or
        device.software_rx_queue.dequeued != 137 or device.packets_dispatched != 125 or
        device.udp_packets_dispatched != 124 or device.peer_mismatch_udp_packets_dropped != 4 or
    ...
    return .{
        ...
        .valid_routed = valid_routed,
        .batch_state = accepted.poll.state,
        .batch_examined = accepted.poll.examined,
        .batch_rejected = accepted.poll.rejected,
        .batch_resolved = batch_resolved,
        .sampled_once = sampled_once,
        .no_retry = no_retry,
        .peer_drop_preserved = peer_drop_preserved,
...
fn verifyNtpWrongOriginateReply(
    device: *Device,
    counter: *time_reference.ContinuousCounter,
) ?NtpWrongOriginateReplyReport {
    if (device.udp_endpoint_count != 2 or device.next_ephemeral_udp_port != 49_210 or
        device.next_udp_generation != 69 or device.tx_producer != 5 or
        device.next_udp_identification != 122 or device.next_dns_transaction_id != 8 or
        completionQueueEnqueued(&tx_completion_queue) != 149 or
        completionQueueDequeued(&tx_completion_queue) != 149 or
        completionQueueEnqueued(&rx_completion_queue) != 22 or
        device.software_rx_queue.enqueued != 131 or device.software_rx_queue.dequeued != 131 or
        device.packets_dispatched != 119 or device.udp_packets_dispatched != 118 or
        device.peer_mismatch_udp_packets_dropped != 4 or
        counter.frequency_hz == 0 or counter.counter_bits == 0)
    {
        return null;
    ...
    const socket = service.client.socket;
    if (socket.endpoint_index != 2 or socket.generation != 69 or socket.local_port != 49_210 or
        device.udp_endpoint_count != 3 or device.next_ephemeral_udp_port != 49_211 or
        device.next_udp_generation != 70)
        return null;
    ...
    transmit_next_cursors[0] = initial_tx.completion.next_cursor;
    if (initial.state != .awaiting or initial.start_reason != .initial or
        initial_tx.identification != 122 or initial_tx.completion.descriptor_index != 5 or
        initial_tx.completion.next_cursor != 6)
        return null;
    ...
    const same_socket_preserved = std.meta.eql(service.client.socket, socket);
    if (!recovery_started or !recovery_switched or !same_socket_preserved or
        recovery_tx.identification != 123 or recovery_tx.completion.descriptor_index != 6 or
        recovery_tx.completion.next_cursor != 7)
        return null;

    const peer_drops_before = device.peer_mismatch_udp_packets_dropped;
    const clock_before_wrong = service.clock;
    const request_before_wrong = service.request;
    const wrong_originate = service.request.client_timestamp +| 1;
    const wrong_originate_routed = enqueueNtpServiceResponse(
        device,
        socket,
        source_pool.servers[2],
        wrong_originate,
    ...
    if (!wrong_originate_routed or device.packets_dispatched != 121 or
        device.udp_packets_dispatched != 120 or device.peer_mismatch_udp_packets_dropped != peer_drops_before)
        return null;

    const wrong = stepNtpServiceAutomatic(device, &service, counter, recovery_tick, 0, 1) orelse return null;
    const wrong_originate_pending = wrong.state == .awaiting and wrong.poll.state == .pending and
        wrong.poll.response == null and wrong.transmit == null;
    const wrong_originate_no_sample = wrong.sample_tick == null;
    const wrong_originate_no_quality = wrong.quality_result == null;
    const wrong_originate_no_step = wrong.step_result == null;
    const wrong_originate_no_apply = wrong.apply_result == null;
    const wrong_originate_clock_preserved = std.meta.eql(service.clock, clock_before_wrong);
    const wrong_originate_request_preserved = std.meta.eql(service.request, request_before_wrong) and
    ...
    const peer_drop_preserved = device.peer_mismatch_udp_packets_dropped == peer_drops_before;
    if (!wrong_originate_pending or wrong.poll.examined != 1 or wrong.poll.rejected != 1 or
        !wrong_originate_no_sample or !wrong_originate_no_quality or !wrong_originate_no_step or
        !wrong_originate_no_apply or !wrong_originate_clock_preserved or
        !wrong_originate_request_preserved or !peer_drop_preserved)
        return null;
    ...
    const overflow = completionQueueOverflow(&tx_completion_queue) + completionQueueOverflow(&rx_completion_queue);
    if (device.udp_endpoint_count != 2 or device.next_ephemeral_udp_port != 49_211 or
        device.next_udp_generation != 70 or device.next_udp_identification != 124 or
        device.next_dns_transaction_id != 8 or device.tx_producer != 7 or
        device.tx_submissions != submissions_before + 2 or txe != 151 or txd != 151 or rxe != 22 or
        overflow != 0 or device.software_rx_queue.enqueued != 134 or
        device.software_rx_queue.dequeued != 134 or device.packets_dispatched != 122 or
        device.udp_packets_dispatched != 121 or device.peer_mismatch_udp_packets_dropped != 4 or
        service.quality_accepted != 2 or service.quality_rejected != 0 or
        service.step_accepted != 2 or service.step_rejected != 0 or service.requests_started != 7 or
        service.retries != 4 or service.responses != 2 or service.retry_limit_hits != 4 or
        service.recovery_limit_hits != 1 or service.recovery_successes != 1 or service.source_rotations != 3)
    ...
    return .{
        ...
        .recovery_switched = recovery_switched,
        .same_socket_preserved = same_socket_preserved,
        .wrong_originate_routed = wrong_originate_routed,
        .wrong_originate_examined = wrong.poll.examined,
        .wrong_originate_rejected = wrong.poll.rejected,
        .wrong_originate_pending = wrong_originate_pending,
        .wrong_originate_no_sample = wrong_originate_no_sample,
        .wrong_originate_no_quality = wrong_originate_no_quality,
        .wrong_originate_no_step = wrong_originate_no_step,
        .wrong_originate_no_apply = wrong_originate_no_apply,
        .wrong_originate_clock_preserved = wrong_originate_clock_preserved,
        .wrong_originate_request_preserved = wrong_originate_request_preserved,
        .peer_drop_preserved = peer_drop_preserved,
        .accepted_quality_result = accepted_quality_result,
        .accepted_step_result = accepted_step_result,
        .accepted_sample_tick = accepted_sample_tick,
