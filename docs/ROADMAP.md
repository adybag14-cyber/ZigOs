...
- The COM1 QEMU driver uses a dedicated loopback TCP chardev, drains all buffered socket lines per iteration, polls at 1 ms, and treats 90 seconds as an inactivity limit refreshed by output and command progress rather than an absolute session wall-clock cap.
...
- Promoted a deterministic single-CPU x86-64 UEFI service/storage boot using the 24-bit ACPI PM fallback to a mandatory GitHub Actions step and uploads `service-user.elf` beside `BOOTX64.EFI`; full network regressions remain separate release gates.
...
- Expanded the canonical COM1 session to 39 commands; both live-network and offline profiles pass locally, including blocking wait and default forced kill, and the source-contract verifier rejects syscall narrowing, stale help, incomplete test-graph and CI-gate regressions.
...
- Locally verified Windows `BOOTX64.EFI`: 2,820,608 bytes, SHA-256 `24C450E9F1BA1ED2D008982B59D2AF155B27DA043AE13A05E330F22ACCE976C6`.
