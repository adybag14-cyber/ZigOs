...
- G258 makes shell command status explicit control data. Foreground children contribute their exact 32-bit kernel wait status; shell-generated failures use bounded 0/1/2/127 values, and the `status` builtin exposes only the immediately previous parsed command status. This status is not an authorization signal and is not persisted. G259 now uses it only as the bounded control input for `&&`/`||`; sequential lists remain separate.
- G259 treats conditional parsing as a side-effect boundary. The shell validates the complete four-command-maximum `&&`/`||` list before running its first segment, so malformed trailing/consecutive operators or overflow cannot partially execute a valid prefix. Operators are recognized without requiring spaces and have intentionally simple equal-precedence, left-to-right semantics. A skipped branch retains the current status and cannot mutate state because it never reaches `execute`; syntax failures return status 2. This is not quoting, expansion, semicolon sequencing, pipelines or job control, and conditional status must not be treated as an authorization decision.
