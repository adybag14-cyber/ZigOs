...
The model covers the post-UEFI kernel, retained CPL3 processes, the bounded RAM VFS, the retained read-only `/boot` FAT volume, the `/persist` A/B journal, COM1/TTY input, the retained e1000e and NVMe paths, and the generated ABI 1.17 SDK interfaces. The legacy i686 environment is outside this document.
...
- Request/completion/pass and queued/completed-page counters are conserved at shutdown; dedicated QEMU requires a nonzero five-page asynchronous proof, while other profiles may remain validly idle.
...
- owner/group/other mode checks and process umask are enforced against UID plus one primary GID; setuid/setgid metadata is stored but deliberately inert at execution, while supplementary groups, credential-changing syscalls, `chown`, set-ID execution, setgid-directory inheritance and sticky-directory semantics remain unimplemented;
