...
- G261 treats pipeline process-group membership as spawn-time process metadata rather than a post-spawn race. ABI 1.21 exposes only two restricted `spawnv` modes: create a child-led pipeline group or join an existing same-parent/same-session sibling leader; arbitrary userspace process-group reassignment remains outside this interface and G141 stays open. The kernel validates the requested leader before installing a runnable child context, and missing/foreign groups fail closed. Shell pipeline syntax is fully bounded to four stages and multi-stage builtins are rejected before the first child starts. This PGID is coordination state, not an authorization credential. G261 also deliberately leaves every stage on inherited shell descriptors and leaves the shell's TTY group foreground, so it does not claim data isolation, pipeline confidentiality, job-control correctness, foreground group waiting or terminal signal routing; G262/G263 must establish those separately.
