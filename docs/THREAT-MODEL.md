...
- owner/group/other mode checks, process umask and sticky-directory removal are enforced against UID plus one primary GID; setuid/setgid metadata is stored but deliberately inert at execution, while supplementary groups, credential-changing syscalls, `chown`, set-ID execution and setgid-directory inheritance remain unimplemented;
