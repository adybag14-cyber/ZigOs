...
The verified EFI GPT partition is now retained after boot and mounted at `/boot` through a dedicated read-only FAT16 backend. Directory metadata is imported into bounded VFS dentries, but file contents remain on NVMe and are streamed through a generic block callback; backed logical bytes are reported separately from RAM-resident VFS bytes. The backend accepts at most 32 files, 24 directories and depth 8, handles 8.3 short names only, validates every imported cluster chain with loop bitmaps, caches the active FAT sector and keeps a sequential per-file cluster cursor. The canonical host image includes root `README.TXT`, nested `EFI/BOOT/BOOT.CFG` and a multi-cluster large-file test; required diagnostic QEMU lists both directory levels, reads both files and stats `BOOTX64.EFI`. Offline and live reports each show three files, two directories, four file reads, zero failures and drained I/O locking; persistent normal reports metadata-only scanning, while diskless normal requires the backend to remain absent and uses the embedded fallback. FAT writes, LFN entries, mirror validation, cross-link detection/repair and general fsck remain open.
