...
uint32_t zigos_main(size_t argc, const uintptr_t *argv, const uintptr_t *envp, const zigos_auxv_entry *auxv) {
    ...
    const uint64_t network_caps = abi.capabilities & (ZIGOS_CAP_UDP_SOCKETS | ZIGOS_CAP_TCP_SOCKETS);
    if ((abi.maximum_sockets == 0 && network_caps != 0) ||
        (abi.maximum_sockets != 0 && network_caps != (ZIGOS_CAP_UDP_SOCKETS | ZIGOS_CAP_TCP_SOCKETS))) {
        return fail(0xC3, "ABI socket capabilities");
    }
    if (!emit("c-sdk: ABI 1.27 discovery passed\r\n")) {
        return 0xC4;
    }

    int64_t self_pid = (int64_t)zigos_syscall6(ZIGOS_SYS_GETPID, 0, 0, 0, 0, 0, 0);
    if (self_pid <= 0 || zigos_kill((uint32_t)self_pid, 0) != ZIGOS_ERRNO_INVALID) {
        return fail(0xC5, "kill invalid-signal validation");
    }
    ...
    if (!emit("c-sdk: generated header/library/device/ioctl/stat/statfs/stattimes/statowner/umask/setid-metadata/flock/lockrange/watchdir/kill/directory-openat/fsync/fdatasync/symlink/readlink/link/nlink/fallocate/sparse/readv/writev passed\r\n")) {
        return 0xD7;
    }
    return 0x57;
}
