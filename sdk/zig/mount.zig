...
pub export fn zigos_main(
    ...
    if (argc != 1) {
        zigos.writeAll(2, "usage: mount\r\n") catch {};
        return status_usage;
    }

    const fd = zigos.open("/proc/mounts", .{ .read = true }, 0) catch |err| return printError(err);
...
fn printError(err: zigos.Error) u32 {
    const message = switch (err) {
        error.NotFound => "mount: procfs unavailable\r\n",
        error.PermissionDenied, error.AccessDenied => "mount: permission denied\r\n",
        error.InputOutput => "mount: input/output error\r\n",
        else => "mount: error\r\n",
    };
