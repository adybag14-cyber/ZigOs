...
## Current release: Capstone 12.0

ZigOs Capstone 12.0 completes twenty-three new legacy i686 goals across hierarchical FAT12 path resolution, per-process current directories, bounded directory creation/removal, direct path I/O, same-directory rename, cross-directory movement, deterministic cluster reclamation, and nested two-boot persistence. A disk-loaded `PATHS.ELF` executes 31 ring-3 syscalls to create `/HOME/DOCS` and `/HOME/ARCHIVE`, normalize absolute, relative, dot, parent, and repeated-separator paths, persist a 600-byte `LOG.TXT` across clusters `33 -> 34`, reject a nonempty directory removal, and prove cluster 36 is reclaimed and reused before `WRITER.ELF` stores `NOTES.TXT` at `36 -> 37`. The second boot independently validates directory sectors, `.` and `..` links, both FAT copies, exact file hashes, tombstones, and free-cluster state while performing zero writes. The release reaches 86 cumulative verified goals. See [`docs/CAPSTONE-12.0.md`](docs/CAPSTONE-12.0.md) for the exact bounded contract and [`docs/CAPSTONE-11.0.md`](docs/CAPSTONE-11.0.md) for the inherited process hierarchy.
