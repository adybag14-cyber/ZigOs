...
## Current release: Capstone 10.0

ZigOs Capstone 10.0 completes eighteen new legacy i686 goals across process memory, clock services, filesystem namespace calls, pipe-based IPC, descriptor duplication, process signals, and PCI discovery. A disk-loaded `SERVICE.ELF` executes 30 ring-3 syscalls to grow and contract `brk`, map and unmap anonymous memory, query parent and uptime state, sleep on real PIT interrupts, stat/rename/unlink a temporary FAT12 file, transfer an exact payload through a process-owned pipe, exercise `dup` and `dup2`, and consume a pending signal. The kernel independently proves every mapping, descriptor, pipe endpoint, FAT cluster, root entry, and signal is cleaned before the inherited persistent writer runs. PCI configuration mechanism 1 now enumerates bus 0 through ports `0xCF8/0xCFC` and validates the QEMU i440FX host bridge. The two-boot contract retains `NOTES.TXT` at its new deterministic `17 -> 18` chain, giving 44 cumulative verified goals. See [`docs/CAPSTONE-10.0.md`](docs/CAPSTONE-10.0.md) for the exact eighteen-goal contract and [`docs/CAPSTONE-9.0.md`](docs/CAPSTONE-9.0.md) for the inherited VM/threading foundation.
