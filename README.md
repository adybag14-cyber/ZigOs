...
- G258 keeps ABI 1.20 and completes shell exit-status propagation without adding new kernel interfaces. Every parsed command returns one `u32` shell status: foreground external commands propagate the exact `WaitStatus.exit_status`, successful builtins return 0, builtin/runtime failures return 1, usage errors return 2, and failed executable lookup returns 127. A dedicated `status` builtin reports the previous command value and then succeeds with 0. The required normal QEMU session proves `hello -> 42`, missing command `-> 127`, failed `cd -> 1`, and successful `pwd -> 0`, while PID 1 still reaps shell PID 2 with status 0. `&&`/`||`, semicolon lists and shell variable syntax remain G259/G260/G267 rather than being folded into this milestone;
