...
## Current release: Capstone 13.0

ZigOs Capstone 13.0 completes twenty-seven new legacy i686 goals across bounded asynchronous process creation, private address spaces and privilege stacks, timer-driven round-robin execution, user yield and sleep, blocking and nonblocking child-status APIs, deterministic wait ordering, directed signal delivery, orphan adoption, init auto-reaping, one-shot wait rejection, and exact resource restoration. A disk-loaded `ASYNC.ELF` launches `WORKA.ELF` and `WORKB.ELF`; `WORKA.ELF` launches `LEAF.ELF`, creating a three-generation process tree. The kernel proves real IRQ0 preemption, an all-sleeping idle context, exit order `A -> B -> LEAF`, adoption of the orphaned leaf by PID 1, automatic leaf reaping, private CR3/code/stack/kernel-stack identities, and restoration of the parent CR3, TSS `esp0`, PID, descriptors, and all twelve temporary frames. The release reaches 113 cumulative verified goals. See [`docs/CAPSTONE-13.0.md`](docs/CAPSTONE-13.0.md) for the exact bounded contract and [`docs/CAPSTONE-12.0.md`](docs/CAPSTONE-12.0.md) for the inherited hierarchical FAT12 layer.
