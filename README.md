...
Local Windows validation is complete for the canonical build, all 95 unique isolated-test declarations, the source contract, the 54-command offline runtime, the 56-command live-network runtime, the x86-64 NVMe four-boot fsync/fdatasync persistence proof, persistent, quarantined-media, recoverable-read-error and diskless normal-boot profiles, and the legacy i686 two-boot regression. The required hosted workflow includes these gates plus cross-platform byte comparison.
...
ZigOs boot FAT: block-backed yes files/directories 3/2 bytes 5570628 metadata/file/block reads 49/4/51 failures 0 clusters claimed/loop/cross/range 10884/0/0/0 lock tickets/outstanding 5/0 quarantine state/reason/events no/none/0 clean yes
...
ZigOs boot FAT: block-backed yes files/directories 3/2 bytes 5518916 metadata/file/block reads 49/0/49 failures 0 clusters claimed/loop/cross/range 10783/0/0/0 lock tickets/outstanding 1/0 quarantine state/reason/events no/none/0 clean yes
...
- **Portable Linux:** clean bootstrap, asset generation, formatting, 95 unique isolated declarations, directly linked Zig/C SDK, init, shell and DNS verification, x86-64 UEFI build, portable PE verification and artifact upload.
- **Windows integration:** clean build, isolated checks, reduced fallback boot, a uniprocessor serial-only network profile, the 54-command offline and 56-command live-network permanent COM1 sessions, the x86-64 NVMe four-boot fsync/fdatasync proof, persistent, cross-linked quarantine, recoverable NVMe read-error and diskless normal boots, and the legacy i686 build/two-boot regression.
