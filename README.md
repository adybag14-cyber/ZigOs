...
## Current release: Capstone 15.0

ZigOs Capstone 15.0 returns the main x86-64 UEFI path to the foreground with sixty-four new verified goals. The canonical build now assembles a deterministic 2,628-byte long-mode userspace workload, wraps and independently verifies it as a 10,240-byte two-segment ELF64 executable, embeds it into the kernel build, parses it again inside ZigOs, maps RX text plus RW/NX data, BSS, stack, heap, and anonymous pages, enables hardware NX through `EFER.NXE`, and executes fifty successful-path `int 0x80` calls at CPL3. The bounded service proves process identity and status, page-by-page pointer validation, output and hashing, two-page `brk`, anonymous mapping, descriptors, pipes, `dup`/`dup2`, signals, deterministic yield/sleep/clock semantics, two recoverable real page faults, and exact descriptor, pipe, mapping, CR3, and frame-allocator restoration. The project reaches 209 cumulative verified goals (`0xD1`). The scope remains one synchronous fixed-layout ELF64 service process rather than a general POSIX process scheduler or dynamic linker. See [`docs/CAPSTONE-15.0.md`](docs/CAPSTONE-15.0.md) for the exact contract and [`docs/CAPSTONE-14.0.md`](docs/CAPSTONE-14.0.md) for the unchanged legacy i686 process-generation release.
