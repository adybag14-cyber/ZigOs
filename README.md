...
ZigOs boot FAT: block-backed yes files/directories 3/2 bytes 5599812 metadata/file/block reads 111/4/113 failures 0 clusters claimed/free/loop/cross/range 10941/5170/0/0/0 lock tickets/outstanding 5/0 quarantine state/reason/events no/none/0 clean yes
...
ZigOs boot FAT: block-backed yes files/directories 3/2 bytes 5548612 metadata/file/block reads 111/0/111 failures 0 clusters claimed/free/loop/cross/range 10841/5270/0/0/0 lock tickets/outstanding 1/0 quarantine state/reason/events no/none/0 clean yes
ZigOs live pseudo filesystems: dev/proc/net registrations 3/5/4 publications 3/5/4 withdrawals 0/0/0 failures 0/0/0 clean yes
...
- ABI 1.17 `umask` syscall 124 stores one nine-bit process creation mask, returns the previous mask, rejects bits outside `0777` failure-atomically, inherits the mask across child spawn, and applies `requested_mode & ~umask` exactly once to `open`/`openat(O_CREAT)` and `mkdir`; chmod, existing objects, hard links and fixed-mode symlinks are unchanged, and child spawn now preserves the parent UID/GID; supplementary groups, `chown`, setuid/setgid and sticky-directory rules remain open;
