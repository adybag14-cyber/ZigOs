...
- freestanding Zig and C SDKs with generated ABI structures, a SysV AMD64 startup/syscall shim, typed file/process/VM/poll/UDP/filesystem wrappers, a bounded DNS A resolver, environment/auxiliary helpers, a directly linked `/bin/init.elf`, and independently verified `/bin/sdk.elf`, `/bin/fs.elf`, standalone `/bin/ls.elf`, `/bin/cat.elf`, `/bin/echo.elf`, `/bin/pwd.elf`, `/bin/mkdir.elf`, `/bin/rm.elf`, `/bin/rmdir.elf`, `/bin/mv.elf`, `/bin/cp.elf`, `/bin/stat.elf`, `/bin/head.elf`, `/bin/tail.elf`, `/bin/wc.elf`, `/bin/grep.elf`, `/bin/hexdump.elf`, `/bin/ps.elf`, `/bin/kill.elf`, `/bin/sleep.elf`, `/bin/mount.elf`, `/bin/df.elf`, `/bin/fsck.elf`, `/bin/uname.elf`, `/bin/env.elf`, `/bin/edit.elf`, `/bin/dns.elf` and `/bin/c-sdk.elf` programs;
...
ZigOs persistent VFS: ... backing alloc/release/fail 55/55/0/0 ... mapped refs/pin/unpin/refresh/fail 0/1/1/1/0 ... page-write tickets/outstanding 378/0 ... clean yes
...
ZigOs persistent VFS: ... backing alloc/release/fail 60/60/0/0 ... mapped refs/pin/unpin/refresh/fail 0/1/1/1/0 ... page-write tickets/outstanding 378/0 ... clean yes
...
ZigOs boot FAT: block-backed yes files/directories 3/2 bytes 5795396 metadata/file/block reads 113/0/113 failures 0 clusters claimed/free/loop/cross/range 11323/4788/0/0/0 lock tickets/outstanding 1/0 quarantine state/reason/events no/none/0 clean yes
ZigOs live pseudo filesystems: dev/proc/net registrations 3/5/4 publications 3/5/4 withdrawals 0/0/0 failures 0/0/0 clean yes
...
ZigOs boot FAT: block-backed no files/directories 0/0 bytes 0 metadata/file/block reads 50/0/50 failures 0 clusters claimed/free/loop/cross/range 0/0/0/1/0 lock tickets/outstanding 1/0 quarantine state/reason/events yes/cross_link/1 clean yes
ZigOs live pseudo filesystems: dev/proc/net registrations 3/5/4 publications 3/5/4 withdrawals 0/0/0 failures 0/0/0 clean yes
...
ZigOs boot FAT: block-backed yes files/directories 3/2 bytes 5795396 metadata/file/block reads 114/3/115 failures 1 clusters claimed/free/loop/cross/range 11323/4788/0/0/0 lock tickets/outstanding 4/0 quarantine state/reason/events no/none/0 clean yes
ZigOs live pseudo filesystems: dev/proc/net registrations 3/5/4 publications 3/5/4 withdrawals 0/0/0 failures 0/0/0 clean yes
ZigOs normal userspace resources: processes 1 descriptors 0 contexts 0 pages 0 alloc/free 144/144 cache-released 13 storage persistent clean yes
...
- G286 keeps ABI 1.25 and adds standalone freestanding Zig `/bin/df.elf` as a bounded `statfs` client. It accepts zero or one `PATH` operand (default `/`), invokes only the existing path-based `statfs` syscall, and prints filesystem kind, block size, total/free/available blocks, total/free nodes, mount ID and bounded read-only/shared/synthetic flags without allocating file-sized state. Required normal QEMU executes `/bin/df.elf /` by absolute path, requires the stable ramfs structure (`block-size 4096`, `blocks 256`, `nodes 128`, `mount 1`, `flags rw shared-blocks shared-nodes`), and parses the live counters to require `available == free <= total` and `free-nodes <= total-nodes`. Canonical `df.elf` is 7,504 bytes (RX 3,192, SHA-256 `6E5F9BF7A19D62DA7CD6B4991BF94A05BD20B9E9538ABA19BD8827D5CDD4F466`); aggregation across all mounts, human-readable units and option parsing remain outside this milestone.
...
- G288 keeps ABI 1.26 and adds standalone freestanding Zig `/bin/uname.elf` as a bounded read-only system-identity viewer. It accepts no operands, opens only the existing live `/proc/version` pseudo-file, and streams that kernel-formatted value through one fixed 128-byte buffer; it adds no syscall, hostname store, machine-identity mutation, hardware probe or direct kernel-structure access. Required normal QEMU executes `/bin/uname.elf` by absolute path, requires exact `ZigOs 19.0.0 x86_64 persistent runtime`, waits for the shell prompt and requires status 0 while rejecting any `uname:` diagnostic. Canonical `uname.elf` is 6,616 bytes (RX 2,304, SHA-256 `D2F98B45926EDA6CD9325D5999DF0DF8A0BA678E34DCA72BA77EC3649E92F641`); option flags, hostname/domainname fields, release-field selection and platform mutation remain outside this milestone.
- G289 keeps ABI 1.26 and adds standalone freestanding Zig `/bin/env.elf` as a bounded startup-environment viewer. It accepts no operands, reads only the inherited `envp` vector supplied by the kernel startup stack, permits at most the generated eight entries and scans no entry beyond the generated 63-byte payload limit before requiring NUL termination and a nonempty `NAME=VALUE` form. It streams each validated entry directly to stdout in inherited order and performs no path lookup, filesystem access, environment mutation, sorting or child launch. Required normal QEMU executes `/bin/env.elf` by absolute path after the G267 shell override proof and requires `PATH=/bin:/persist`, `HOME=/home/root`, `TERM=zigos`, `SHELL=/bin/sh.elf` plus accepted `G267_A/B/C` values while forbidding rejected `G267_D`, proving the executable observes the real child environment rather than canned defaults. Canonical `env.elf` is 6,320 bytes (RX 2,008, SHA-256 `6BBF3508AD0ACB9D55FFE5837A6EDEE79502E67CB76C18DC6158CDFD1EBE1A62`); assignment syntax, environment removal, sorting and command execution remain outside this milestone.
- G290 keeps ABI 1.26 and adds standalone freestanding Zig `/bin/edit.elf` as a bounded serial-console line editor. It requires exactly one file path, loads an existing regular file into a fixed 4 KiB document buffer (or starts empty when the path does not exist), and exposes only `p`, `a TEXT`, `d`, `w`, `wq` and `q`: print, append one line, delete the last logical line, save, save-and-quit, or quit without saving. A dedicated fixed command reader retains read-ahead across syscalls, frames CR/LF independently of `read()` boundaries, accepts the full 256-byte canonical TTY edit payload and discards any overlong input through its delimiter before another command is interpreted. Save uses the public create/truncate/write descriptor path, `writeAll`, then `fsync` with global `sync` fallback only when file-scoped synchronization is unsupported. Required normal QEMU queues an exact 256-byte append command plus five further commands in one serial burst, requires no framing diagnostic, writes exactly 11 bytes and verifies the final file is `BASE\nFINAL\n` before cleanup. Canonical `edit.elf` is 12,568 bytes (RX 5,024, RW/BSS 4,104, SHA-256 `22BBD827D06A8D18E8FB7B2F6D3662D65C851BF311038068A1635FBC4DE6F1C0`). G290 also raises the live fixed VFS node pool from 96 to 128 (and the derived dentry pool from 192 to 256): the new permanent `/bin/edit.elf` exposed that the eight-record persistence-recovery boot had exhausted the old node ceiling during SDK conformance. The 128-node pool remains bounded and restores recovery/test headroom without changing the public statfs layout. Save currently truncates the selected target before rewriting it rather than performing an atomic temporary-file replacement; cursor movement, arbitrary insertion/deletion, undo, search, syntax modes and files larger than 4 KiB remain outside this milestone.
- G291 keeps ABI 1.26 and strengthens the existing standalone `/bin/cp.elf` rather than adding a duplicate copy command. The descriptor frontend retains G279's source/destination permission, type, mode and same-inode preflight, but its data path now delegates to one allocation-free `copy_core.copy` loop with the existing fixed 512-byte scratch buffer. The core treats a zero-byte read as EOF, repeats reads until EOF, and for every nonempty read loops writes until the complete returned chunk has been accepted; a zero-progress write fails explicitly as `WriteZero`, which `cp` maps to `EIO` instead of spinning forever. A dedicated isolated host root forces every read to at most five bytes and every write to at most three bytes over a deterministic payload, requires exact reconstructed bytes and more write than read calls, then separately proves a zero-progress writer is rejected after exactly one call. The pre-existing normal-QEMU G279 session remains the real VFS integration proof for self-copy protection and successful standalone copy, while G291 makes partial-I/O semantics independently deterministic. Canonical `cp.elf` is 7,560 bytes (RX 3,248, SHA-256 `79BE16EBCFC015108535E734D3618559CECA4985FD10C8F19BA29D82F17D7734`). The copy remains deliberately non-transactional: an I/O/no-space failure after destination truncation can leave a partial target; recursive/multi-source copying, sparse/xattr/timestamp preservation, reflinks, atomic replacement and rollback remain outside this milestone.
...
- **Portable Linux:** clean bootstrap, asset generation, formatting, 104 unique isolated declarations, directly linked Zig/C SDK, init, shell and DNS verification, x86-64 UEFI build, portable PE verification and artifact upload.
