...
ZigOs Capstone 9.0 completes sixteen new legacy i686 goals across virtual memory, fault recovery, kernel threading, IRQ queuing, and writable FAT12 namespace management. The kernel now exposes reusable map/query/unmap operations, proves TLB-coherent remapping, recovers a demand-zero user fault, contains read-only/supervisor/guard-page violations, schedules four kernel threads through sleep and wait-queue states, preserves ordered IRQ1 make/break events in a bounded ring, and proves FAT12 rename, unlink, cluster reclamation, and deterministic reuse without leaving temporary files behind. FAT metadata is mounted into a fully mirrored nine-sector cache, avoiding repeated PIO metadata reads while preserving verified writes to both copies. The established Capstone 8 persistence workload remains intact, giving 26 cumulative release goals and the same two-boot `NOTES.TXT` proof. See [`docs/CAPSTONE-9.0.md`](docs/CAPSTONE-9.0.md) for the exact sixteen-goal contract and [`docs/CAPSTONE-8.0.md`](docs/CAPSTONE-8.0.md) for the writable-userspace foundation.
