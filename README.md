...
## Current release: Capstone 8.0

ZigOs Capstone 8.0 adds persistent writable FAT12 userspace and completes ten release goals: verified ATA PIO writes, multi-cluster files, mirrored FAT allocation, root-entry creation and truncation, seek/append semantics, eight CPL3 syscalls, a disk-loaded writer, generic ELF execution, wait and fault containment, and disk-loaded concurrent ring-3 tasks. `WRITER.ELF` creates the 720-byte `NOTES.TXT` at clusters `14 -> 15`, and the two-boot harness proves that the same file and hash survive a second boot without rebuilding or modifying the image. See [`docs/CAPSTONE-8.0.md`](docs/CAPSTONE-8.0.md) for the exact ten-goal and persistence contract; earlier userland, multiprocess, and protected-loader foundations remain documented in [`docs/CAPSTONE-7.0.md`](docs/CAPSTONE-7.0.md), [`docs/CAPSTONE-6.0.md`](docs/CAPSTONE-6.0.md), and [`docs/CAPSTONE-5.0.md`](docs/CAPSTONE-5.0.md).
