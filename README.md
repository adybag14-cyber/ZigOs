...
## Current release: Capstone 14.0

ZigOs Capstone 14.0 completes thirty-two new legacy i686 goals across generation-tagged task handles, stale-handle rejection, reusable task and process slots, terminal tombstone records, scheduled fork-from-current-context, atomic in-place executable replacement, exact descriptor and pipe-reference inheritance, close-on-exec, asynchronous page-fault containment, and multi-generation resource restoration. A disk-loaded `GENRUN.ELF` drives 107 CPL3 syscalls: it executes nine handle-based spawns across a four-slot runtime, forces eight slot reuses, performs one real scheduled fork, replaces the two fork branches with `EXECA.ELF` and `EXECB.ELF`, proves two failed exec attempts are atomic, rejects five stale-handle operations, contains one scheduled page fault, and reclaims ten process generations while restoring the original CR3, TSS `esp0`, PID, descriptor, pipe, process-record, and frame baseline. The release reaches 145 cumulative verified goals. See [`docs/CAPSTONE-14.0.md`](docs/CAPSTONE-14.0.md) for the exact bounded contract and [`docs/CAPSTONE-13.0.md`](docs/CAPSTONE-13.0.md) for the inherited asynchronous scheduler.
